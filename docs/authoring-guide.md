...
**File attachments:** When running under `raymond serve`, `<await>` can also
ask the user to upload files or expose files for them to view. Add attributes
on the opening tag:

```
<await next="REVIEW.md" upload_slots="resume.pdf:application/pdf">
Please upload your résumé as `resume.pdf`.
</await>
```

The runtime stages uploads (and any declared display files) into
`<task folder>/inputs/<input_id>/`. The state immediately following the
await receives a `{{input_id}}` template variable so it can read the files
back. See [workflow-protocol.md](workflow-protocol.md#file-attachments) for
...
`{{task_folder}}` contains the absolute path to the agent's task folder — a
private directory the orchestrator assigns for the agent to write artifacts,
intermediate state, and (in daemon mode) input attachments. The default
layout is `.raymond/tasks/<workflow_id>/<agent_id>`.

```markdown
Write the generated report to `{{task_folder}}/report.md`.
```

### `{{input_id}}` — Identifier of the Resolving Await

`{{input_id}}` is bound **only** in the state running immediately after an
`<await>` resolves. It carries the input ID generated by the runtime when the
await was entered, which is also the name of the per-input subdirectory under
`{{task_folder}}/inputs/`. Use it to read uploaded or staged files:

```markdown
Read uploaded files from `{{task_folder}}/inputs/{{input_id}}/`.
```

The variable is cleared before the next state runs (one transition deep). If
a workflow needs the ID further along, plumb it forward explicitly through
the transition `input` attribute or by writing it to a file in the post-await
state.
...
through the web UI. Declare an upload affordance on the `<await>` tag, then
...
<await next="ANALYZE.md" upload_slots="resume.pdf:application/pdf">
...
The slot mode declaration tells the UI to render a single labeled upload
control for `resume.pdf`, and constrains uploads to PDF content type. For
an open-ended affordance ("attach any supporting files") use bucket mode:

```
<await next="REVIEW.md"
       upload_bucket="true"
       upload_max_count="5"
...
`{{task_folder}}/inputs/{{input_id}}/` in the next state.
