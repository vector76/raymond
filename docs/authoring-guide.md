...
**Context:** Preserved. The LLM session survives the ask (like `goto`), so
...
`<task folder>/asks/<ask_id>/`. The state immediately following the
ask receives a `{{ask_id}}` template variable so it can read the files
...
`{{ask_id}}` is bound **only** in the state running immediately after an
`<ask>` resolves. It carries the input ID generated by the runtime when the
ask was entered, which is also the name of the per-input subdirectory under
`{{task_folder}}/asks/`. Use it to read uploaded or staged files:
...
the transition `input` attribute or by writing it to a file in the post-ask
state.
