...
entry point is resolved from `1_START` (or `START` as a fallback) inside that
...
5. Resolve entry point: directory scope → `1_START` or `START` (any supported
   extension); `.md` specifier → the specified file; zip → `1_START` or `START`
   (any supported extension) inside the archive.
