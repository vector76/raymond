...
|-----------------|-------------|
| `<goto>`, `<call>`, `<function>`, `<call-workflow>`, `<function-workflow>`, `fork-next`, `fork-workflow-next`, `<ask>` | Solid `-->` |
| `<fork>`, `<fork-workflow>`, `<reset>`, `<reset-workflow>`, return edges, `<ask>` timeout | Dashed `-.->` |

**`<ask>` edges:** An `<ask>` transition produces a solid edge labeled
`ask` from the current state to the `next` target. If `timeout_next` is
specified, a second dashed edge labeled `ask timeout` is drawn to the timeout
target. Both edges from frontmatter declarations and body-parsed `<ask>` tags
are rendered.
...
- *Traced returns* (`<call>`, `<function>`) — raymond traces which states inside
  ...
  `return (caller-name)`. The tracing follows `<goto>`, `<reset>`, and `<ask>`
