...
func TestParseAskBasic(t *testing.T) {
	out := `<ask next="TARGET.md">prompt text</ask>`
	transitions, err := parsing.ParseTransitions(out)
	require.NoError(t, err)
	require.Len(t, transitions, 1)
	tr := transitions[0]
	assert.Equal(t, "ask", tr.Tag)
	assert.Equal(t, "TARGET.md", tr.Target)
	assert.Equal(t, "prompt text", tr.Payload)
	assert.Equal(t, "TARGET.md", tr.Attributes["next"])
}

func TestParseAskAllAttributes(t *testing.T) {
	out := `<ask next="A.md" timeout="24h" timeout_next="B.md">Please review the document.</ask>`
	transitions, err := parsing.ParseTransitions(out)
	require.NoError(t, err)
	require.Len(t, transitions, 1)
	tr := transitions[0]
...
func TestParseAskMissingNextAttribute(t *testing.T) {
	out := `<ask>prompt text</ask>`
	_, err := parsing.ParseTransitions(out)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "requires a non-empty \"next\" attribute")
}

func TestParseAskEmptyNextAttribute(t *testing.T) {
	out := `<ask next="">prompt text</ask>`
	_, err := parsing.ParseTransitions(out)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "requires a non-empty \"next\" attribute")
}

func TestParseAskMultilinePrompt(t *testing.T) {
	out := "<ask next=\"REVIEW.md\">Please review:\n\n- **Item 1**: Check formatting\n- **Item 2**: Verify links\n\nThanks!</ask>"
	transitions, err := parsing.ParseTransitions(out)
	require.NoError(t, err)
	require.Len(t, transitions, 1)
	tr := transitions[0]
...
func TestParseAskAlongsideOtherTransition(t *testing.T) {
	out := "<goto>NEXT.md</goto>\n<ask next=\"WAIT.md\">Hold on</ask>"
	transitions, err := parsing.ParseTransitions(out)
	require.NoError(t, err)
	require.Len(t, transitions, 2)
	assert.Equal(t, "goto", transitions[0].Tag)
	assert.Equal(t, "NEXT.md", transitions[0].Target)
	assert.Equal(t, "ask", transitions[1].Tag)
	assert.Equal(t, "WAIT.md", transitions[1].Target)
	assert.Equal(t, "Hold on", transitions[1].Payload)
}

func TestParseAskRejectsPathSeparatorInNext(t *testing.T) {
	out := `<ask next="subdir/file.md">prompt</ask>`
	_, err := parsing.ParseTransitions(out)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "contains path separator")
}

func TestParseAskRejectsBackslashInNext(t *testing.T) {
	out := `<ask next="sub\file.md">prompt</ask>`
	_, err := parsing.ParseTransitions(out)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "contains path separator")
}

func TestParseAskEmptyContent(t *testing.T) {
	out := `<ask next="TARGET.md"></ask>`
	transitions, err := parsing.ParseTransitions(out)
	require.NoError(t, err)
	require.Len(t, transitions, 1)
	tr := transitions[0]
