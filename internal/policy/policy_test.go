...
// ----------------------------------------------------------------------------
// Ask policy validation
// ----------------------------------------------------------------------------

func TestValidateAskMatchesValidTransition(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md"},
		},
	}
	tr := parsing.Transition{
		Tag:        "ask",
...
func TestValidateAskAbstractNextMatches(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "NEXT"},
		},
...
func TestValidateAskWrongNextRejects(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md"},
		},
...
func TestValidateAskTimeoutNextMatches(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md", "timeout_next": "TIMEOUT.md"},
		},
	}
	tr := parsing.Transition{
		Tag:        "ask",
...
func TestValidateAskTimeoutNextWrongRejects(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md", "timeout_next": "TIMEOUT.md"},
		},
...
func TestValidateAskTimeoutNextAbstractMatches(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET", "timeout_next": "TIMEOUT"},
		},
	}
	tr := parsing.Transition{
		Tag:        "ask",
...
// Ask must be excluded from implicit transitions.
func TestCannotUseImplicitAsk(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md"},
		},
...
func TestGetImplicitTransitionRejectsAsk(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md"},
		},
...
// Reminder prompt renders ask entries correctly.
func TestGenerateReminderAskBasic(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md"},
		},
	...
	out, err := policy.GenerateReminderPrompt(p)
	require.NoError(t, err)
	assert.Contains(t, out, `<ask next="TARGET.md">[human-facing prompt here]</ask>`)
}

func TestGenerateReminderAskWithTimeoutAttributes(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask", "next": "TARGET.md", "timeout": "300", "timeout_next": "TIMEOUT.md"},
		},
	}
	out, err := policy.GenerateReminderPrompt(p)
	require.NoError(t, err)
	// Attributes are sorted: next, timeout, timeout_next
	assert.Contains(t, out, `<ask next="TARGET.md" timeout="300" timeout_next="TIMEOUT.md">[human-facing prompt here]</ask>`)
}

// Mixed policy with ask and non-ask entries validates correctly.
func TestValidateMixedPolicyAskAndGoto(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "goto", "target": "NEXT.md"},
...
func TestGenerateReminderMixedWithAsk(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "goto", "target": "NEXT.md"},
			{"tag": "ask", "next": "REVIEW.md"},
	...
	require.NoError(t, err)
	assert.Contains(t, out, "<goto>NEXT.md</goto>")
	assert.Contains(t, out, `<ask next="REVIEW.md">[human-facing prompt here]</ask>`)
	assert.Contains(t, out, "<result>...</result>")
}

// Ask without next constraint in policy allows any next target.
func TestValidateAskNoNextConstraintAllowsAny(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "ask"},
		},
...
// Ask tag not in allowed list produces helpful error.
func TestValidateAskNotAllowedShowsAllowedTags(t *testing.T) {
	p := &policy.Policy{
		AllowedTransitions: []map[string]string{
			{"tag": "goto", "target": "NEXT.md"},
		},
