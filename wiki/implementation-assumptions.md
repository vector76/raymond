...
- `<function>FILE.md</function>` - Pattern 1 (stateless/pure)
...
indicates same-context continuation. `<call>` evokes subroutine semantics.
`<function>` suggests stateless evaluation. `<fork>` matches Unix fork()
semantics (independent child process).
...
Subsequent `<goto>` tags resume the existing session.

## Default Model

**Assumption:** Use the default Claude Code model (currently Sonnet) unless
explicitly specified. Evaluators (Pattern 1) should specify `model="haiku"` in
the prompt or via a convention.

**Rationale:** Let Claude Code manage model selection by default. Override only
when cost/speed tradeoffs matter (evaluators).

**Alternative considered:** Always specify model, or configure per-workflow.
...
The prompt for forked workflows should instruct the AI to include this tag.
